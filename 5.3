...
legend ('sin(X)','cos(X)','sin(X)+cos(X)','sin(X)-cos(X)','location','southwest')
title('Trigonometric Functions with Fixed axis and Legend') % see Figure 5.68.
gtext('sinX') % identifies each curve with a text string
gtext('cos(X)')
gtext('sin(X)+cos(X)')
gtext('sin(X)-cos(X)') % see Figure 5.69 where the curves are identified by texts
...
stairs(X,Y2)
title('Cos(X) VS X')
% plots of :Sin(X) vs X, and Cos(X) vs X using a stair case approximation
% see Figure 5.71
